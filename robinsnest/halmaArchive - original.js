...
var geometry,pieceGeometry, material, mesh;
...
function drawBoard() {
	...
	try{
		document.getElementById('Test').clearRect(0, 0, kPixelWidth, kPixelHeight);
	}catch(e){
		createNew = true;
	}
	...
	camera = new THREE.PerspectiveCamera(35,kPixelWidth / kPixelHeight,0.1,1000);
	//camera = new THREE.OrthographicCamera(kPixelWidth/-2,kPixelWidth/-2,kPixelHeight/-2,kPixelHeight/-2,1,1000);	
	...
	geometry = new THREE.CubeGeometry(kPixelWidth,kPixelHeight,50);
	material = new THREE.MeshBasicMaterial({color: 0xE6E6E6, wireframe : false});
	...
	renderer.setSize(kPixelWidth,kPixelHeight);	
	
	
	if(createNew){
	var domEl = renderer.domElement
	domEl.id = 'Test';
	domEl.addEventListener("click", halmaOnClick, false);
	//domEl.setAttribute("onClick","halmaOnClick(this)");
	document.getElementById('gameArea').appendChild(domEl);
	}else{
		var domEl = renderer.domElement
		domEl.id = 'Test';
		domEl.addEventListener("click", halmaOnClick, false);
...
function drawPiece(p, selected) {
	...
	//alert(column + "," + row);
	mesh = new THREE.Mesh(new THREE.CylinderGeometry(radius, radius, 50, 50, 10, false), material);;
    
	mesh.rotation.x = (90 *(Math.PI/180));
	mesh.position.x = tdCoordsX[column];
	mesh.position.y = tdCoordsY[row];
	mesh.position.z = 20;
	scene.add( mesh );
...
function initGame(canvasElement, moveCountElement) {
    tdCoordsY=[200,150,100,50,0,-50,-100,-150,-200]
	tdCoordsX=[-200,-150,-100,-50,0,50,100,150,200];
	if (!canvasElement) {
...
function toggleCameraMove(){
		var currVal = document.getElementById('cameraMoving').value;
		...
		if (currVal == true){
			...
			camera.position.z = 750;
			drawBoard();
			document.getElementById('cameraMoving').value = 'false';
		}else{
			controls.enabled = true;
			document.getElementById('cameraMoving').value = 'true';
		}
	}
